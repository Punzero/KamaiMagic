...
class KamaiGuardianJourney:
    def __init__(self, root: tk.Tk):
        ...
        choice = simpledialog.askstring(
            "KAMAI NOC LOGIN", prompt_text, parent=self.root
        )
    ...
    def setup_journey_artifacts(self):
        ...
        if self.role == "SENIOR":
            ...
            with open(hint_path, "w", encoding="utf-8") as f:
                ...
                f.write(
                    "[DEBUG] Token: 4b414d41495f4d414749435f53595354454d5f4f56455252494445\n"
                )
    ...
    def action(self):
        ...
        if k == self.GUI_PASSWORD:
            messagebox.showinfo(
                "Status", "NOC Clearance Granted. Deleting Journey Artifacts..."
            )
            self.cleanup()
            self.root.destroy()
        ...
        # 3. Junior WIN Trigger (Triggered by the result of the HEX token)
            ...
            self.drop_reward(
                "You force-pushed to main-Bold strategy.jpg", "JUNIOR_RECOVERY_COMPLETE"
            )
            messagebox.showinfo(
                "Easter Egg", "JUNIOR RECOVERY SUCCESS!\nReward dropped on Desktop."
            )
    ...
    def build_authentic_ui(self):
        ...
        tk.Label(
            t1, text=time.strftime("%m/%d/%Y %H:%M:%S"), fg="white", bg=self.PURPLE_BG
        ).pack()
        ...
        self.lk1.bind(
            "<Button-1>",
            lambda e: webbrowser.open("https://www.youtube.com/@KamaiSanCh"),
        )

        self.lk2 = tk.Label(
            self.root,
            text="How to buy bitcoins?",
            fg="#add8e6",
            bg=self.PURPLE_BG,
            cursor="hand2",
            font=link_f,
        ...
        self.lk2.bind(
            "<Button-1>", lambda e: webbrowser.open("https://x.com/KaMaiT4Chi")
        )
        ...
        self.lk3.bind(
            "<Button-1>",
            lambda e: webbrowser.open("https://facebook.com/KaMaiTaChi0san"),
        )
        ...
        tk.Label(
            logo, text="₿ bitcoin", font=("Arial", 14, "bold"), fg="#f7931a", bg="white"
        ).pack(pady=10)
        tk.Label(logo, text="ACCEPTED HERE", font=("Arial", 8), bg="white").pack()

        self.tip_lbl = tk.Label(
            footer,
            text="Send $300 worth of MAGIC to this address:",
            fg="yellow",
            bg=self.PURPLE_BG,
            font=("Arial", 9, "bold"),
        )
        ...
        self.ent = tk.Entry(
            footer, bg="#e0e0e0", fg="black", font=("Arial", 11), relief="flat"
        )
        self.ent.insert(0, self.addr)
        self.ent.config(state="readonly")
        self.ent.place(x=170, y=35, width=310, height=25)

        tk.Button(footer, text="Copy", command=self.copy_address).place(
            x=485, y=35, width=40, height=25
        )

        self.btn_check = tk.Button(
            self.root, text="Check Payment", width=34, command=self.action
        )
        self.btn_check.place(x=270, y=560)
        self.btn_decrypt = tk.Button(
            self.root, text="Decrypt", width=34, command=self.action
        )
    ...
    def cleanup(self):
        files = [
            "NOC_CONFIG.kamai",
            "NOC_GATEWAY_ACCESS.txt",
            "MAGIC_YAIMAK.jpg",
            "IT_TICKET.kamai",
            "GIT_RECOVERY_LOG.txt",
            "You force-pushed to main-Bold strategy.jpg",
        ]
        for f in files:
